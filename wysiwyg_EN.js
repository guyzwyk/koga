...
function generate_wysiwyg(textareaID) {
  ...
  toolbar2 += '<td>&nbsp;</td></tr></table>';  
	...
	// Create iframe which will be used for rich text editing ***** The width qnd height of the wysiwyg here px/em/%
	var iframe = '<table cellpadding="0" cellspacing="0" border="0" style="background-color:#FFF; width:' + wysiwygWidth + 'px; height:' + wysiwygHeight + 'px;border: 1px solid #000;"><tr><td valign="top">\n'
...
function formatText(id, n, selected) {
  ...
  document.getElementById("wysiwyg" + n).contentWindow.focus();
	...
	if (viewTextMode == 1 && disabled_id == 1) {
      ...
      window.open(popupsDir + 'insert_image.html?wysiwyg=' + n,'popup','location=0,status=0,scrollbars=0,resizable=0,width=500,height=300');
	  }
  ...
  var browserName = navigator.appName;
	...
	if (browserName == "Microsoft Internet Explorer") {	  
	  ...
	  /*var div = document.getElementById('wysiwyg' + n).contentWindow.document.createElement("div");
		...
		var p = document.getElementById('wysiwyg' + n).contentWindow.document.createElement("p");
		 
		p.innerHTML = html;
		var node = insertNodeAtSelection(p, n);
...
function outputFontSelect(n) {
  ...
  var FontSelectOn         = FontSelectObj[3];
	...
	FontSelectDropDown[n] += '<span id="Fonts' + n + '" class="admin-dropdown" style="width: 145px;">';
